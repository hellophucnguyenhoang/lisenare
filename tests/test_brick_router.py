...
def test_get_next_brick_without_brick_id(client: TestClient):
    existing_learner = Learner(id=2, setting=LearnerSetting(practice_lang="en"))
    app.dependency_overrides[auth_service.decode_token_get_learner] = lambda: (
        existing_learner
    )
...
def test_get_next_brick_with_specific_id_ignores_practice_lang(client: TestClient):
    with Session(engine) as session:
        brick_en = session.exec(
            select(Brick).where(Brick.creator_id == 2, Brick.target_lang == "en")
        ).first()
